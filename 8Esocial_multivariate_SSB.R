...
# ==============================================================================
# 2. IDENTIFY AFFILIATIVE BEHAVIORS WITH SUFFICIENT VARIATION
# ==============================================================================

cat("\n=== IDENTIFYING AFFILIATIVE BEHAVIORS WITH SUFFICIENT VARIATION ===\n")
...
# ==============================================================================
# 4. STANDARDIZE SOCIAL CONNECTEDNESS BEHAVIOR VARIABLES
# ==============================================================================

cat("\n=== STANDARDIZING SOCIAL CONNECTEDNESS BEHAVIOR VARIABLES ===\n")
...
cat("Standardizing", length(good_behaviors_in_data), "social connectedness behavior variables:\n")
...
cat("\n*** All social connectedness behavior variables now standardized (mean=0, SD=1) ***\n")
...
cat("Correlation matrix of standardized social connectedness behaviors:\n")
print(round(correlation_matrix, 3))

# Visualize correlation matrix
corrplot(correlation_matrix, method = "color", type = "upper", 
         order = "hclust", tl.cex = 0.7, tl.col = "black",
         title = "Correlation Matrix: Standardized Social Connectedness Behaviors",
         mar = c(0,0,1,0))
...
# ==============================================================================
# 6. FIT MULTIVARIATE MODEL WITH ALL SOCIAL CONNECTEDNESS BEHAVIORS
# ==============================================================================

cat("\n=== FITTING MULTIVARIATE MODEL WITH ALL SOCIAL CONNECTEDNESS BEHAVIORS ===\n")
...
if (!is.null(multivariate_model)) {
  ...
  # Extract coefficients for social connectedness behaviors
  coef_table <- model_summary$coefficients$cond
  behavior_coefs <- coef_table[standardized_behaviors, , drop = FALSE]
  
  cat("\n=== SOCIAL CONNECTEDNESS BEHAVIOR EFFECTS (CONTROLLING FOR ALL OTHER BEHAVIORS) ===\n")
  cat("*** Effects shown as log-scale coefficients for standardized predictors ***\n")
  cat("*** Each effect controls for all other social connectedness behaviors ***\n")
  ...
  cat("\n=== SIGNIFICANT SOCIAL CONNECTEDNESS BEHAVIOR EFFECTS (p < 0.05) ===\n")
  ...
  if(nrow(significant_effects) > 0) {
    ...
    cat("No significant social connectedness behavior effects found in multivariate model\n")
  }
  ...
  tryCatch({
    ...
    # Plot diagnostics
    plot(sim_residuals, main = "Multivariate Social Connectedness Model Diagnostic Plot")
  ...
  backwards_stepwise <- function(full_model, data, alpha = 0.05) {
  ...
  stepwise_results <- backwards_stepwise(multivariate_model, analysis_data, alpha = 0.05)
  ...
  print(stepwise_results$elimination_log)
  
  # Compare models
  cat("\n=== MODEL COMPARISON ===\n")
  cat("Full model AIC:", round(AIC(multivariate_model), 2), "\n")
  cat("Final model AIC:", round(stepwise_results$final_aic, 2), "\n")
  cat("AIC improvement:", round(AIC(multivariate_model) - stepwise_results$final_aic, 2), "\n")
  
  # Show final model
  final_model <- stepwise_results$final_model
  final_predictors <- stepwise_results$final_predictors
  
  cat("\nFinal model predictors (", length(final_predictors), "):\n")
  cat(paste(gsub("_std", "", final_predictors), collapse = ", "), "\n")
  
  cat("\n=== FINAL STEPWISE MODEL RESULTS ===\n")
  final_summary <- summary(final_model)
  print(final_summary)
  
  # Extract final model coefficients
  final_coef_table <- final_summary$coefficients$cond
  final_behavior_coefs <- final_coef_table[final_predictors, , drop = FALSE]
  
  cat("\n=== FINAL MODEL: SOCIAL CONNECTEDNESS BEHAVIOR EFFECTS ===\n")
  
  # Create final results table
  final_results_df <- data.frame(
    Behavior = gsub("_std$", "", rownames(final_behavior_coefs)),
    Estimate = final_behavior_coefs[, "Estimate"],
    SE = final_behavior_coefs[, "Std. Error"],
    Z_value = final_behavior_coefs[, "z value"],
    P_value = final_behavior_coefs[, "Pr(>|z|)"],
    Significant = final_behavior_coefs[, "Pr(>|z|)"] < 0.05
  )
  
  # Calculate percentage changes
  final_results_df$Percent_Change <- (exp(final_results_df$Estimate) - 1) * 100
  
  # Clean behavior names
  final_results_df$Behavior_Clean <- gsub("rate_", "", final_results_df$Behavior)
  final_results_df$Behavior_Clean <- gsub("_", " ", final_results_df$Behavior_Clean)
  final_results_df$Behavior_Clean <- tools::toTitleCase(final_results_df$Behavior_Clean)
  
  # Sort by p-value
  final_results_df <- final_results_df[order(final_results_df$P_value), ]
  
  print(final_results_df[, c("Behavior_Clean", "Estimate", "SE", "Percent_Change", "P_value", "Significant")])
  
  # ==============================================================================
  # COMPARE WITH YOUR INDEPENDENT SIGNIFICANT MODEL
  # ==============================================================================
  
  cat("\n=== COMPARISON WITH INDEPENDENT SIGNIFICANT MODELS ===\n")
  
  # If you have specific behaviors that were significant in univariate tests,
  # fit those as a comparison model
  cat("To compare with your independent significant model, specify the variables here:\n")
  cat("Example: significant_vars <- c('rate_behavior1_std', 'rate_behavior2_std')\n")
  
  # Placeholder for your significant variables - MODIFY THIS
  # significant_vars <- c("rate_your_significant_behavior1_std", "rate_your_significant_behavior2_std")
  
  # Uncomment and modify this section once you specify your significant variables:
  
  # if(exists("significant_vars") && length(significant_vars) > 0) {
  #   cat("\nFitting your independent significant model...\n")
  #   
  #   independent_formula <- as.formula(paste("adjusted_SSB ~", 
  #                                          paste(significant_vars, collapse = " + "), 
  #                                          "+ (1|MotherID)"))
  #   
  #   independent_model <- glmmTMB(
  ...
  #   cat("Full model AIC:", round(AIC(multivariate_model), 2), "\n")
  #   cat("Stepwise model AIC:", round(AIC(final_model), 2), "\n")
  #   cat("Independent significant model AIC:", round(AIC(independent_model), 2), "\n")
  #   
  #   # Best model
  #   aics <- c(Full = AIC(multivariate_model), 
  #            Stepwise = AIC(final_model), 
  #            Independent = AIC(independent_model))
  #   
  ...
  #   aic_diffs <- aics - min(aics)
  #   for(i in 1:length(aic_diffs)) {
  #     cat(names(aic_diffs)[i], ":", round(aic_diffs[i], 2), "\n")
  #   }
  # }
  ...
  cat("\n=== FINAL STEPWISE MODEL DIAGNOSTICS ===\n")
  
  tryCatch({
    final_sim_residuals <- simulateResiduals(final_model)
    
    final_disp_test <- testDispersion(final_sim_residuals)
    final_uniform_test <- testUniformity(final_sim_residuals)
    final_zero_test <- testZeroInflation(final_sim_residuals)
    
    cat("Final model diagnostics:\n")
    cat("Dispersion test p-value:", round(final_disp_test$p.value, 4), "\n")
    cat("Uniformity test p-value:", round(final_uniform_test$p.value, 4), "\n")
    cat("Zero-inflation test p-value:", round(final_zero_test$p.value, 4), "\n")
    
    if(final_disp_test$p.value > 0.05 & final_uniform_test$p.value > 0.05 & final_zero_test$p.value > 0.05) {
      cat("✓ Final model diagnostics look good!\n")
    } else {
      cat("⚠ Some diagnostic issues detected in final model\n")
    }
    
    # Plot final model diagnostics
    plot(final_sim_residuals, main = "Final Stepwise Model Diagnostic Plot")
    
  }, error = function(e) {
    cat("Final model diagnostic testing failed:", e$message, "\n")
  })
  ...
  cat("\n=== MULTIVARIATE SOCIAL CONNECTEDNESS MODEL INTERPRETATION ===\n")
  cat("This model tests the independent effects of each social connectedness behavior\n")
  cat("while controlling for all other social connectedness behaviors simultaneously.\n")
  cat("This approach accounts for the intercorrelated nature of social behaviors\n")
  ...
  cat("1. Controls for correlations between different social connectedness behaviors\n")
  cat("2. Identifies independent effects of each behavior\n")
  cat("3. Reduces risk of false positives from multiple testing\n")
  cat("4. Provides a more comprehensive view of social effects on sons' SSB\n\n")
  
  cat("Results should be interpreted as:\n")
  cat("'The effect of a 1 standard deviation increase in [social behavior],\n")
  cat("holding all other social connectedness behaviors constant'\n")
  
  cat("\n=== STEPWISE ELIMINATION COMPLETE ===\n")
  cat("Final model saved as 'final_model'\n")
  cat("Use summary(final_model) to see detailed results\n")
  
} else {
  cat("Multivariate model failed to converge. Consider:\n")
...
install.packages("gt")
# Create Nature-style table using gt package
library(gt)
library(dplyr)
...
multivariate_results <- data.frame(
  Predictor = c("Intercept", 
                ...
                "Teeth chatter"),
  Estimate = c(1.148, -0.571, 0.253, 0.098, -0.235, 0.293, 0.483, 0.046, -0.183, 0.244, 0.459),
  SE = c(0.242, 0.382, 0.337, 0.342, 0.273, 0.210, 0.159, 0.335, 0.283, 0.312, 0.179),
  z_value = c(4.752, -1.496, 0.752, 0.287, -0.861, 1.397, 3.032, 0.136, -0.648, 0.781, 2.570),
  P_value = c("<0.001", "0.135", "0.452", "0.774", "0.389", "0.162", "0.002", "0.892", "0.517", "0.435", "0.010"),
  Significant = c(TRUE, FALSE, FALSE, FALSE, FALSE, FALSE, TRUE, FALSE, FALSE, FALSE, TRUE)
)

# Create Nature-style gt table
nature_gt_table <- multivariate_results %>%
  select(-Significant) %>%  # Remove helper column
  gt() %>%
  
  # Table title and subtitle
  tab_header(
    title = "Table 1. Multivariate model of maternal behaviors predicting same-sex sexual behaviour",
    subtitle = "Effects of standardised maternal social behaviors on male offspring SSB"
  ) %>%
  ...
  tab_style(
    ...
    locations = cells_body(rows = c(7, 11))  # Initiating contact and Teeth chatter
  ) %>%
  
  # Highlight significant P-values
  tab_style(
    style = cell_text(weight = "bold", color = "black"),
    locations = cells_body(columns = P_value, rows = c(1, 7, 11))
  ) %>%
  
  # Add footnote
  tab_footnote(
    footnote = "Generalised linear mixed model with Tweedie distribution and log link function. All behavioural predictors standardized to z-scores (mean = 0, SD = 1). Models weighted by proportion of sons sampled per mother. Random intercepts for MotherID included (variance = 1.98e-10). AIC = 86.7. n = 38 observations from 27 mothers."
  ) %>%
...
# Load gt package (gtsave is part of gt)
library(gt)
library(webshot2)

# Now try the export
gtsave(nature_gt_table, "table1_multivariate.png", expand = 10)

nature_gt_table %>% 
  gtsave("table1_multivariate.html")

# Alternative: More compact version for manuscript
compact_table <- multivariate_results %>%
  mutate(
    `P-value` = case_when(
      P_value == "<0.001" ~ "<0.001***",
      P_value == "0.002" ~ "0.002**", 
  ...
  select(Predictor, Estimate, SE, `z-value` = z_value, `P-value`) %>%
  gt() %>%
  tab_header(title = "Multivariate GLMM: Maternal behaviors and offspring SSB") %>%
  fmt_number(columns = c(Estimate, SE, `z-value`), decimals = 3) %>%
  tab_style(
    style = cell_text(weight = "bold"),
    locations = cells_body(rows = c(7, 11))
  ) %>%
